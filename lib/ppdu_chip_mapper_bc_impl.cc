...
static const float d_cck_dqpsk_phase[4][2]={
    {0,M_PI},{1.5*M_PI,0.5*M_PI},{0.5*M_PI,1.5*M_PI},{M_PI,0}
};
...
static const gr_complex d_append_symbols[11] = {
    gr_complex(1,0),gr_complex(-1,0),gr_complex(1,0),
    gr_complex(1,0),gr_complex(-1,0),gr_complex(1,0),
    gr_complex(1,0),gr_complex(1,0),gr_complex(-1,0),
    gr_complex(-1,0),gr_complex(-1,0)};

ppdu_chip_mapper_bc::sptr ppdu_chip_mapper_bc::make(const std::string& lentag)
{
    return gnuradio::make_block_sptr<ppdu_chip_mapper_bc_impl>(lentag);
}
 ...
 */
ppdu_chip_mapper_bc_impl::ppdu_chip_mapper_bc_impl(const std::string& lentag)
...
int ppdu_chip_mapper_bc_impl::nout_check() const
{
    switch(d_rate){
    case LONG1M:
        return 88;
...
int ppdu_chip_mapper_bc_impl::cck_5_5M_chips(gr_complex* out, uint8_t byte, bool even)
{
    ...
    uint8_t d2 = (byte>>2) & 0x01;
    uint8_t d3 = (byte>>3) & 0x01;
    phase_b2 = (d3)? 1.5*M_PI : 0.5*M_PI;
    phase_b4 = (d2 != d3)? M_PI : 0;
    
    uint8_t q2 = (byte>>6) & 0x01;
    uint8_t q4 = (byte>>7) & 0x01;
    phase_q2 = (q4)? 1.5*M_PI : 0.5*M_PI;
    phase_q4 = (q2 != q4)? M_PI : 0;
...
int ppdu_chip_mapper_bc_impl::cck_11M_chips(gr_complex* out, uint8_t byte, bool even)
{
...
int ppdu_chip_mapper_bc_impl::dqpsk_2M_chips(gr_complex* out, uint8_t byte, bool even)
{
    gr_complex tmpChip;
    for(int i=0;i<4;++i){
...
int ppdu_chip_mapper_bc_impl::dbpsk_1M_chips(gr_complex* out, uint8_t byte, bool even)
{
    gr_complex tmpChip;
    for(int i=0;i<8;++i){
...
int ppdu_chip_mapper_bc_impl::chip_gen(gr_complex* out, 
                                       ...
                                       int& nconsume)
{
    ...
    while(nconsume<nin && nout<noutput_items && d_copy<d_count){
        if( (d_preamble_type && (d_copy< (LONG_PREAMBLE_LENGTH+WIFI80211DSSS_PHYHDR_BYTES) ) )||
            (!d_preamble_type && (d_copy<SHORT_PREAMBLE_LENGTH) ) ){
...
bool ppdu_chip_mapper_bc_impl::update_rate(unsigned char raw)
{
    if(raw == 0){
        d_rate = LONG1M;
        ...
        d_preamble_type = true;
        d_psdu_symbol_num = 8;
        dout << "RATE[LONG1M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
    }else if(raw == 1){
        d_rate = LONG2M;
        ...
        d_preamble_type = true;
        d_psdu_symbol_num = 4;
        dout << "RATE[LONG2M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
    }else if(raw == 2){
        d_rate = LONG5_5M;
        ...
        d_preamble_type = true;
        d_psdu_symbol_num = 2;
        dout << "RATE[LONG5.5M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
    }else if(raw == 3){
        d_rate = LONG11M;
        ...
        d_preamble_type = true;
        d_psdu_symbol_num = 1;
        dout << "RATE[LONG11M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
    }else if(raw == 4){
        d_rate = SHORT2M;
        d_rate_tag = pmt::intern("SHORT2M");
        d_rateVal = 2;
        d_chip_mapper = &ppdu_chip_mapper_bc_impl::dqpsk_2M_chips;
        d_preamble_type = false;
        d_psdu_symbol_num = 4;
        dout << "RATE[SHORT2M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
    }else if(raw == 5){
        d_rate = SHORT5_5M;
        ...
        d_preamble_type = false;
        d_psdu_symbol_num = 2;
        dout << "RATE[SHORT5.5M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
    }else if(raw == 6){
        d_rate = SHORT11M;
        ...
        d_preamble_type = false;
        d_psdu_symbol_num = 1;
        dout << "RATE[SHORT11M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
    }else{
        d_rate = LONG1M;
        d_rate_tag = pmt::intern("LONG1M");
        d_rateVal = 1;
        ...
        d_preamble_type = true;
        d_psdu_symbol_num = 8;
        dout << "RATE[DEFAULT LONG1M], Number of Symbols[" << d_psdu_symbol_num << "]" << std::endl;
        return false;
    }
    return true;
}

int ppdu_chip_mapper_bc_impl::update_tag(int total_bytes) const
{
    switch(d_rate){
    case LONG1M:
    ...
    case SHORT11M:
        return SHORT_PREAMBLE_LENGTH *88+
               WIFI80211DSSS_PHYHDR_BYTES*44 +
               (total_bytes - SHORT_PREAMBLE_LENGTH - WIFI80211DSSS_PHYHDR_BYTES )*8;
    break;
    default:
        return 0;
    break;
...
void ppdu_chip_mapper_bc_impl::forecast(int noutput_items,
...
int ppdu_chip_mapper_bc_impl::general_work(int noutput_items,
                                           ...
                                           gr_vector_void_star& output_items)
{
    ...
    if(d_copy<d_count){
        ...
        int ncon = 0;
        nout = chip_gen(out,in,noutput_items,nin,ncon);
        if(d_count==d_copy){
            d_append = 0;
        }
        consume_each(ncon);
